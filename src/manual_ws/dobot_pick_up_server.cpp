...
void execute_pick_up(const dobot::DobotGoalConstPtr& goal, Server* as)
{
    ...
    ros::Publisher action_pub = n.advertise<std_msgs::Int64>("control_success", 1);

// 设置参数

    node.param("control_x", control_x, 230.0);    
    node.param("control_y", control_y, 0.0);   
    node.param("control_z", control_z, 30.0);    
...
// 创建PTP客户端
        ...
        do {
            ...
            srv1.request.x = control_x;
            srv1.request.y = control_y;
            srv1.request.z = control_z;
            srv1.request.r = 0;
            client.call(srv1);  
